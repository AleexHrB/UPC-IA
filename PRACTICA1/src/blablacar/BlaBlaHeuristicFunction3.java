 ...
 * @author Walter J.T.V
 */
public class BlaBlaHeuristicFunction3 implements HeuristicFunction{

    /**
     * @param state This is a non-instanciated state 
     * @return The sum of the remaining distance to work  for each User in the system, this distance is defined as D(i,j) = |ix - jx| + |iy - jy|
     */
    public double getHeuristicValue(ArrayList<ArrayList<XYLocation>> state){
        ...
        double h = 0.0;
        for(int i = 0; i < n; ++i){
            h = h + ManhattanDistance(CurrentLocations.get(i), WorkLocations.get(i));
        }
